...
type CompanyRequest = { id: string; company_name: string; contact_name: string; contact_email: string; contact_phone: string | null; expected_user_count: number | null; activity_type: string | null; notes: string | null; status: string; created_at: string; review_note: string | null; invitation_sent_at: string | null; invitation_activated_at: string | null; invitation_cancelled_at: string | null; approved_company_id: string | null; manager_full_name: string | null; manager_email: string | null; latest_email_status: string | null };
type ApprovalForm = { companySlug: string; managerFullName: string; managerEmail: string; managerPassword: string; planKey: string };
type RequestNote = { id: string; note_text: string; created_at: string; created_by_name: string | null };
...
function requestStatusLabel(status: string) { return ({ submitted: "قيد المراجعة", awaiting_info: "بانتظار معلومات", approved: "تم الاعتماد", invitation_sent: "الدعوة أُرسلت", manager_activated: "المدير فعّل حسابه", rejected: "مرفوض", cancelled: "الدعوة ملغاة" } as Record<string, string>)[status] ?? status; }
function requestStatusColor(status: string) { return ["rejected", "cancelled"].includes(status) ? palette.error : status === "manager_activated" ? palette.success : ["approved", "invitation_sent"].includes(status) ? palette.primary : palette.warning; }
...
export default function PlatformPortalScreen() {
  ...
  const requestApi = async <T,>(path: string, body: Record<string, unknown> = {}) => {
  ...
  const startApproval = (request: CompanyRequest) => {
    setMessage(null); setError(null); setReviewRequest(null); setInfoRequest(null);
    if (!request.activity_type?.trim() || !request.expected_user_count || request.expected_user_count < 1) {
      setError("لا يمكن الاعتماد قبل اكتمال قائمة الفحص: طبيعة النشاط وعدد المستخدمين المتوقع مطلوبان. استخدم «طلب معلومات» أولاً.");
      return;
    }
  ...
  const requestInfo = async () => {
    if (!infoRequest || infoNote.trim().length < 3) { setError("اكتب المعلومات المطلوبة بوضوح قبل الإرسال."); return; }
    setSubmitting(true); setError(null);
    try {
      const result = await requestApi<{ emailSent?: boolean; warning?: string | null }>(`/api/platform/company-requests/${infoRequest.id}/request-info`, { informationNeeded: infoNote });
      setMessage(result.emailSent ? `تم طلب المعلومات من ${infoRequest.company_name} وإرسال الرسالة بالبريد.` : `تم حفظ طلب المعلومات. ${result.warning ?? "تعذر إرسال البريد."}`);
      setInfoRequest(null); setInfoNote(""); await load();
    } catch (reason) { setError(reason instanceof Error ? reason.message : "تعذر طلب المعلومات."); }
    setSubmitting(false);
  };

  const toggleNotes = async (requestId: string) => {
    const next = expandedRequest === requestId ? null : requestId;
    setExpandedRequest(next); setNoteDraft("");
    if (!next || notesByRequest[requestId] || !supabase) return;
    const { data, error: notesError } = await supabase.rpc("tips_crm_list_company_request_notes", { target_request_id: requestId });
    if (notesError) { setError("تعذر تحميل سجل الملاحظات."); return; }
    setNotesByRequest((current) => ({ ...current, [requestId]: (data ?? []) as RequestNote[] }));
  };

  const addNote = async (requestId: string) => {
    if (!noteDraft.trim()) { setError("اكتب الملاحظة أولاً."); return; }
    setSubmitting(true); setError(null);
    try {
      await requestApi(`/api/platform/company-requests/${requestId}/notes`, { noteText: noteDraft });
      const { data } = await supabase!.rpc("tips_crm_list_company_request_notes", { target_request_id: requestId });
      setNotesByRequest((current) => ({ ...current, [requestId]: (data ?? []) as RequestNote[] })); setNoteDraft(""); setMessage("تمت إضافة الملاحظة إلى السجل الداخلي.");
    } catch (reason) { setError(reason instanceof Error ? reason.message : "تعذر حفظ الملاحظة."); }
    setSubmitting(false);
  };

  const resendInvitation = async (request: CompanyRequest) => {
    if (!request.approved_company_id) return;
    setSubmitting(true); setError(null);
    try { await requestApi(`/api/platform/companies/${request.approved_company_id}/resend-invitation`); setMessage(`أُعيد إرسال رابط إعداد الحساب إلى ${request.manager_email || "مدير الشركة"}.`); await load(); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "تعذر إعادة إرسال الدعوة."); }
    setSubmitting(false);
  };

  const cancelInvitation = (request: CompanyRequest) => Alert.alert("إلغاء دعوة المدير", "سيُعطّل حساب المدير غير المفعّل ولن يتمكن من الدخول. هل تريد المتابعة؟", [{ text: "رجوع", style: "cancel" }, { text: "إلغاء الدعوة", style: "destructive", onPress: () => void (async () => { setSubmitting(true); try { await requestApi(`/api/platform/company-requests/${request.id}/cancel-invitation`); setMessage("تم إلغاء دعوة إعداد حساب مدير الشركة."); await load(); } catch (reason) { setError(reason instanceof Error ? reason.message : "تعذر إلغاء الدعوة."); } setSubmitting(false); })() }]);
  ...
  const pendingRequests = useMemo(() => requests.filter((request) => ["submitted", "awaiting_info"].includes(request.status)), [requests]);
  const pendingInvitations = useMemo(() => requests.filter((request) => ["approved", "invitation_sent"].includes(request.status)).length, [requests]);
  ...
  if (!profile?.is_platform_admin) return <Redirect href={"/platform/login" as never} />;
  ...
  return <ScreenContainer className="px-5" containerClassName="bg-background"><ScrollView contentContainerStyle={[styles.content, isWide && styles.wideContent]} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
    <AppHeader eyebrow="إدارة مركزية · جميع الشركات" title="بوابة مدير المنصة" right={<TouchableOpacity onPress={() => { void signOut(); router.replace("/login" as never); }} style={styles.signOut}><MaterialIcons name="logout" size={17} color={palette.error} /><Text style={styles.signOutText}>خروج</Text></TouchableOpacity>} />
    <View style={styles.hero}><View style={styles.heroIcon}><MaterialIcons name="account-tree" size={25} color="#FFFFFF" /></View><View style={{ flex: 1, flexShrink: 1, alignItems: "flex-end" }}><Text style={styles.heroTitle}>أهلاً {profile.full_name}</Text><Text style={styles.heroText}>تابع دورة الشركة من المراجعة حتى تفعيل حساب مديرها الأول.</Text></View></View>
    <View style={styles.metrics}><MetricCard label="قيد المراجعة" value={String(pendingRequests.length)} icon="pending-actions" tone="amber" /><MetricCard label="دعوات غير مكتملة" value={String(pendingInvitations)} icon="mail-outline" tone="blue" /><MetricCard label="شركات نشطة" value={String(companies.filter((company) => company.status === "active").length)} icon="domain" /></View>
    ...
    <SectionTitle title="طلبات الشركات ودورة الانضمام" action={`${requests.length} طلب`} />
    {requests.length ? <View style={isWide ? styles.wideGrid : undefined}>{requests.map((request) => <View style={[styles.requestCard, isWide && styles.wideCard]} key={request.id}><View style={styles.cardTop}><View style={styles.requestIcon}><MaterialIcons name="business-center" size={20} color={requestStatusColor(request.status)} /></View><View style={{ flex: 1, flexShrink: 1, alignItems: "flex-end" }}><View style={styles.titleRow}><Text style={styles.cardTitle}>{request.company_name}</Text><View style={[styles.status, { backgroundColor: `${requestStatusColor(request.status)}18` }]}><Text style={[styles.statusText, { color: requestStatusColor(request.status) }]}>{requestStatusLabel(request.status)}</Text></View></View><Text style={styles.cardMeta}>{request.contact_name} · {request.contact_email}</Text><Text style={styles.cardMeta}>رقم الطلب: {request.id.slice(0, 8).toUpperCase()} · {dateArabic(request.created_at)}</Text></View></View><View style={styles.lifecycleRow}><Text style={[styles.lifecycleText, !request.activity_type && styles.missingText]}>النشاط: {request.activity_type || "مطلوب"}</Text><Text style={[styles.lifecycleText, !request.expected_user_count && styles.missingText]}>المستخدمون: {request.expected_user_count || "مطلوب"}</Text></View>{request.notes ? <Text style={styles.notes}>{request.notes}</Text> : null}{request.review_note ? <Text style={styles.reviewNote}>{request.review_note}</Text> : null}{request.manager_email ? <Text style={styles.inviteState}>المدير: {request.manager_email} · {request.invitation_activated_at ? "تم التفعيل" : request.invitation_sent_at ? "الدعوة مرسلة" : "بانتظار الإرسال"}{request.latest_email_status === "failed" ? " · آخر إرسال تعذر" : ""}</Text> : null}<View style={styles.cardActions}>{["submitted", "awaiting_info"].includes(request.status) ? <><TouchableOpacity onPress={() => { setReviewRequest(request); setInfoRequest(null); setReviewNote(""); setError(null); }} style={styles.rejectButton}><Text style={styles.rejectText}>رفض</Text></TouchableOpacity><TouchableOpacity onPress={() => { setInfoRequest(request); setReviewRequest(null); setInfoNote(""); setError(null); }} style={styles.requestInfoButton}><Text style={styles.requestInfoText}>طلب معلومات</Text></TouchableOpacity><TouchableOpacity onPress={() => startApproval(request)} style={styles.approveButton}><MaterialIcons name="how-to-reg" size={17} color="#FFFFFF" /><Text style={styles.approveText}>فحص واعتماد</Text></TouchableOpacity></> : null}{["approved", "invitation_sent"].includes(request.status) && !request.invitation_cancelled_at ? <><TouchableOpacity onPress={() => cancelInvitation(request)} style={styles.rejectButton}><Text style={styles.rejectText}>إلغاء الدعوة</Text></TouchableOpacity><TouchableOpacity onPress={() => void resendInvitation(request)} style={styles.approveButton}><MaterialIcons name="mark-email-unread" size={17} color="#FFFFFF" /><Text style={styles.approveText}>إعادة الإرسال</Text></TouchableOpacity></> : null}<TouchableOpacity onPress={() => void toggleNotes(request.id)} style={styles.notesButton}><Text style={styles.notesButtonText}>{expandedRequest === request.id ? "إخفاء الملاحظات" : "الملاحظات"}</Text></TouchableOpacity></View>{expandedRequest === request.id ? <View style={styles.notePanel}><Text style={styles.notePanelTitle}>سجل الملاحظات الداخلي</Text>{(notesByRequest[request.id] ?? []).map((note) => <View style={styles.noteItem} key={note.id}><Text style={styles.noteItemText}>{note.note_text}</Text><Text style={styles.noteItemMeta}>{note.created_by_name || "مدير المنصة"} · {dateArabic(note.created_at)}</Text></View>)}{!(notesByRequest[request.id] ?? []).length ? <Text style={styles.noteEmpty}>لا توجد ملاحظات مسجلة بعد.</Text> : null}<TextInput value={noteDraft} onChangeText={setNoteDraft} multiline textAlign="right" placeholder="اكتب ملاحظة داخلية" placeholderTextColor="#94A39C" style={styles.noteInput} /><TouchableOpacity disabled={submitting} onPress={() => void addNote(request.id)} style={[styles.addNoteButton, submitting && styles.dimmed]}><Text style={styles.addNoteText}>إضافة ملاحظة</Text></TouchableOpacity></View> : null}</View>)}</View> : <View style={styles.empty}><MaterialIcons name="task-alt" size={26} color={palette.success} /><Text style={styles.emptyTitle}>لا توجد طلبات حتى الآن</Text><Text style={styles.emptyText}>تظهر هنا الطلبات التي ترسلها الشركات من نموذج الانضمام.</Text></View>}
...
const styles = StyleSheet.create({
  ...
  signOut: { minHeight: 38, paddingHorizontal: 12, borderRadius: 12, backgroundColor: "#FFF0F0", borderWidth: 1, borderColor: "#F8C4C4", flexDirection: "row-reverse", alignItems: "center", justifyContent: "center", gap: 5 }, signOutText: { color: palette.error, fontSize: 11, fontWeight: "900" },
  hero: { backgroundColor: "#143D35", borderRadius: 21, padding: 17, flexDirection: "row-reverse", gap: 12, alignItems: "center" }, heroIcon: { width: 46, height: 46, borderRadius: 15, backgroundColor: "#28715F", alignItems: "center", justifyContent: "center" }, heroTitle: { color: "#FFFFFF", fontSize: 17, fontWeight: "900", textAlign: "right", lineHeight: 24 }, heroText: { color: "#C6E6DD", fontSize: 12, lineHeight: 18, marginTop: 4, textAlign: "right" },
