...
export default function CompanyTerritorySetupScreen() {
  ...
  return <ScreenContainer className="px-5" containerClassName="bg-background"><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
    ...
    <View style={styles.next}><MaterialIcons name="groups" size={19} color="#7C3AED" /><View style={styles.alignEnd}><Text style={styles.nextTitle}>الخطوة التالية</Text><Text style={styles.nextText}>بعد حفظ المناطق، افتح إعداد الفريق لإسناد منطقة أو أكثر لكل مندوب قبل بدء الزيارات.</Text></View><TouchableOpacity onPress={() => router.push("/company-team-setup" as never)}><Text style={styles.nextLink}>الفريق</Text></TouchableOpacity></View>
