...
export default function AdminTeamPage() {
  ...
  const resetForm = () => {
    setFullName(""); setEmail(""); setTemporaryPassword(""); setRoleKey("sales_rep"); setTerritoryLabel(""); setForcePasswordChange(true);
  };
  ...
  const createAccount = async () => {
    ...
    if (roleKey !== "sales_manager" && !territoryLabel) { Alert.alert("المنطقة مطلوبة", "اختر منطقة تغطية للمندوب من المناطق المعتمدة."); return; }
    setCreating(true);
    try {
      const response = await fetch(`${getApiBaseUrl()}/api/employee-accounts`, {
        method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${session.access_token}` },
        body: JSON.stringify({ fullName, email, password: temporaryPassword, roleKey, territoryLabel, forcePasswordChange }),
      });
