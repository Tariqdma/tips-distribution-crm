...
export default function AdminTeamPage() {
  const { data, resendInvite, revokeInvite, setTeamMemberTerritories } = useCrm();
  ...
  const createAccount = async () => {
    ...
    const territoryLabels = territoryOptions.filter((territory) => territoryIds.includes(territory.id)).map((territory) => territory.name);
    setCreating(true);
    try {
      const response = await fetch(`${getApiBaseUrl()}/api/employee-accounts`, {
        method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${session.access_token}` },
        body: JSON.stringify({ fullName, email, password: temporaryPassword, roleKey, territoryId: territoryIds[0], territoryIds, territoryLabel: territoryLabels.join("، "), forcePasswordChange }),
      });
