...
export function AdminDashboard() {
  ...
  return <ScreenContainer className={isPhone ? "px-4" : "px-5"} containerClassName="bg-background"><ScrollView contentContainerStyle={[styles.content, isWide && styles.wideContent]} showsVerticalScrollIndicator={false}>
    ...
    <View style={styles.quickActions}><TouchableOpacity onPress={() => router.push("/company-setup" as never)} style={styles.quickAction}><MaterialIcons name="settings-suggest" size={19} color={palette.primary} /><Text style={styles.quickActionText}>تهيئة الشركة</Text></TouchableOpacity><TouchableOpacity onPress={() => router.push("/company-team-setup" as never)} style={styles.quickAction}><MaterialIcons name="account-tree" size={19} color={palette.primary} /><Text style={styles.quickActionText}>إعداد الفريق</Text></TouchableOpacity><TouchableOpacity onPress={() => router.push("/team" as never)} style={styles.quickAction}><MaterialIcons name="person-add-alt-1" size={19} color={palette.primary} /><Text style={styles.quickActionText}>دعوة عضو</Text></TouchableOpacity></View>
...
const styles = StyleSheet.create({
  content: { paddingTop: 10, paddingBottom: 28 }, wideContent: { maxWidth: 1160, width: "100%", alignSelf: "center" }, headerActions: { flexDirection: "row", gap: 8, alignItems: "center" }, roleButton: { width: 40, height: 40, borderRadius: 14, backgroundColor: "#E9F8F2", alignItems: "center", justifyContent: "center" }, bannerCopy: { flex: 1, alignItems: "flex-end" },
