...
export function AdminDashboard() {
  ...
  return <ScreenContainer className={isPhone ? "px-4" : "px-5"} containerClassName="bg-background"><ScrollView contentContainerStyle={[styles.content, isWide && styles.wideContent]} showsVerticalScrollIndicator={false}>
    <AppHeader eyebrow={operational.usesServerProfile ? `صلاحية موثقة · ${operational.territory}` : "مسؤولية اليوم · ولاية الخرطوم"} title="لوحة الإدارة" right={<View style={styles.headerActions}><NotificationButton count={unreadNotificationCount} /><TouchableOpacity onPress={() => router.push("/profile" as never)} style={styles.roleButton} accessibilityRole="button" accessibilityLabel="فتح الملف الشخصي"><MaterialIcons name="person" size={19} color={palette.primary} /></TouchableOpacity><TouchableOpacity onPress={() => router.push("/team" as never)} style={styles.roleButton} accessibilityRole="button" accessibilityLabel="إدارة الفريق"><MaterialIcons name="groups" size={19} color={palette.primary} /></TouchableOpacity></View>} />
