...
export default function InviteAcceptanceScreen() {
  const { token } = useLocalSearchParams<{ token: string }>(); const { session, loading, refreshProfile } = useSupabaseAuth(); const [processing, setProcessing] = useState(false); const [message, setMessage] = useState("سجّل الدخول بالحساب الذي وصلته الدعوة للمتابعة.");
  const accept = async () => { if (!token || !supabase || !session) return; setProcessing(true); const { data, error } = await supabase.rpc("tips_crm_accept_invite", { token }); if (error) { setMessage(error.message); setProcessing(false); return; } await refreshProfile(); setMessage(`تم قبول الدعوة وتعيين دورك بنجاح: ${data?.[0]?.role_key ?? "الموظف"}.`); setProcessing(false); };
