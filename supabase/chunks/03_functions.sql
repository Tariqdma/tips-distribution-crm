...
BEGIN
  ...
  VALUES (tips_crm.current_actor_company_id(), auth.uid(), log_action, log_entity_type, log_entity_id, COALESCE(log_details, '{}'::jsonb));
END;
$$;
...
BEGIN
  ...
  SELECT count(*) INTO res_terr_num FROM tips_crm.territories WHERE company_id = tips_crm.current_actor_company_id() AND is_active AND client_key = ANY(coalesce(employee_territory_keys, ARRAY[]::text[]));
  IF res_terr_num <> req_terr_num THEN RAISE EXCEPTION 'One or more territories are unavailable'; END IF;
  UPDATE tips_crm.profiles SET full_name = trim(employee_full_name), email = lower(trim(employee_email)), role_key = employee_role_key, active_company_id = actor_company_id, is_active = true, must_change_password = employee_force_password_change, temporary_password_issued_at = now(), updated_at = now() WHERE id = target_profile_id;
  ...
  IF EXISTS (SELECT 1 FROM tips_crm.company_memberships WHERE company_id = actor_company_id AND profile_id = target_profile_id) THEN
    UPDATE tips_crm.company_memberships SET role_key = employee_role_key, is_active = true, updated_at = now() WHERE company_id = actor_company_id AND profile_id = target_profile_id;
  ELSE
    INSERT INTO tips_crm.company_memberships(company_id, profile_id, role_key, is_active, joined_at, updated_at) VALUES (actor_company_id, target_profile_id, employee_role_key, true, now(), now());
  END IF;

  DELETE FROM tips_crm.territory_assignments WHERE company_id = actor_company_id AND profile_id = target_profile_id AND territory_id NOT IN (SELECT id FROM tips_crm.territories WHERE company_id = actor_company_id AND client_key = ANY(coalesce(employee_territory_keys, ARRAY[]::text[])));
  INSERT INTO tips_crm.territory_assignments(company_id, territory_id, profile_id, assigned_by) SELECT tips_crm.current_actor_company_id(), territory.id, target_profile_id, auth.uid() FROM tips_crm.territories territory WHERE territory.company_id = tips_crm.current_actor_company_id() AND territory.client_key = ANY(coalesce(employee_territory_keys, ARRAY[]::text[])) ON CONFLICT (territory_id, profile_id) DO UPDATE SET company_id = EXCLUDED.company_id, assigned_by = EXCLUDED.assigned_by;
