...
CREATE OR REPLACE FUNCTION tips_crm.log_audit(log_action text, log_entity_type text, log_entity_id text, log_details jsonb DEFAULT '{}'::jsonb)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.tips_crm_sync_account(local_ref text, account_type text, account_name text, account_specialty text, account_state text, account_city text, account_area text, account_address text, account_phone text)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = tips_crm, auth, public
