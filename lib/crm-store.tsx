...
export function CrmProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<CrmContextValue>(() => ({
    ...
    completeVisit: (visitId, completion) => {
      ...
      commit((current) => ({ ...current, visits: current.visits.map((visit) => visit.id === visitId ? { ...visit, status: completion.isInsideTerritory ? "مكتملة" : "تحتاج مراجعة", result: completion.result, note: completion.note, followUpAction: completion.followUpAction?.trim() || undefined, followUpDate: completion.followUpDate || undefined, reportPriority: completion.reportPriority ?? "متوسطة", attachments: completion.attachments, location: completion.location, isInsideTerritory: completion.isInsideTerritory, collectionAmount: completion.collectionAmount ?? 0, revenueAmount: completion.revenueAmount ?? 0, checkedInAt: new Date().toLocaleTimeString("ar", { hour: "2-digit", minute: "2-digit" }), completedAt: new Date().toISOString() } : visit), notifications: [notify("تم توثيق زيارة", `${completion.result}${completion.attachments?.length ? ` · ${completion.attachments.length} مرفقات` : ""}`, "زيارة"), ...(completion.followUpDate ? [notify("متابعة مجدولة", `${account?.name ?? "الجهة"}: ${completion.followUpAction?.trim() || "راجع الخطوة التالية"} في ${completion.followUpDate}.`, "تنبيه")] : []), ...current.notifications] }));
