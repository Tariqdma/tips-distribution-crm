...
import { saveVisitProductContext, type VisitProductContextInput } from "@/lib/visit-product-context";
...
export type MedicalVisitPlace = "عيادة خاصة" | "مستشفى" | "مركز إحالة";
export type MedicalPrescribingLevel = "مرتفع" | "متوسط" | "منخفض" | "غير واصف";
...
export function CrmProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<CrmContextValue>(() => ({
    ...
    completeVisit: async (visitId, completion) => {
      ...
      const payload: OfflineVisitPayload = { result: completion.result, note: completion.note, followUpAction: completion.followUpAction?.trim() || undefined, followUpDate: completion.followUpDate, reportPriority: completion.reportPriority ?? "متوسطة", attachments: completion.attachments, location: completion.location, isInsideTerritory: completion.isInsideTerritory, collectionAmount: completion.collectionAmount ?? 0, revenueAmount: completion.revenueAmount ?? 0, receiptReference: completion.receiptReference?.trim() || undefined, medicalInteractionType: completion.medicalInteractionType, medicalVisitGoal: completion.medicalVisitGoal, medicalVisitPlace: completion.medicalVisitPlace, medicalPrescribingLevel: completion.medicalPrescribingLevel, promotedProduct: completion.promotedProduct?.trim() || undefined, scientificMessage: completion.scientificMessage?.trim() || undefined, doctorInterest: completion.doctorInterest, medicalFeedback: completion.medicalFeedback?.trim() || undefined, productInteractions: completion.productInteractions, sampleDeliveries: completion.sampleDeliveries, marketFeedback: completion.marketFeedback?.trim() || undefined, competitorNotes: completion.competitorNotes?.trim() || undefined, followUpRecommendation: completion.followUpRecommendation?.trim() || undefined };
      commit((current) => ({ ...current, visits: current.visits.map((visit) => visit.id === visitId ? { ...visit, status: completion.isInsideTerritory ? "مكتملة" : "تحتاج مراجعة", result: completion.result, note: completion.note, followUpAction: payload.followUpAction, followUpDate: payload.followUpDate, reportPriority: payload.reportPriority, attachments: payload.attachments, location: payload.location, isInsideTerritory: payload.isInsideTerritory, collectionAmount: payload.collectionAmount, revenueAmount: payload.revenueAmount, receiptReference: payload.receiptReference, medicalInteractionType: payload.medicalInteractionType, medicalVisitGoal: payload.medicalVisitGoal, medicalVisitPlace: payload.medicalVisitPlace, medicalPrescribingLevel: payload.medicalPrescribingLevel, promotedProduct: payload.promotedProduct, scientificMessage: payload.scientificMessage, doctorInterest: payload.doctorInterest, medicalFeedback: payload.medicalFeedback, productInteractions: payload.productInteractions, sampleDeliveries: payload.sampleDeliveries, marketFeedback: payload.marketFeedback, competitorNotes: payload.competitorNotes, followUpRecommendation: payload.followUpRecommendation, checkedInAt: new Date().toLocaleTimeString("ar", { hour: "2-digit", minute: "2-digit" }), completedAt: new Date().toISOString() } : visit), notifications: [...(completion.followUpDate ? [notify("متابعة مجدولة", `${account.name}: ${completion.followUpAction?.trim() || "راجع الخطوة التالية"} في ${completion.followUpDate}.`, "تنبيه")] : []), ...current.notifications] }));
